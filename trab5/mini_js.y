...
ATR : LVALUE_ATR EQUAL RVALUE { if(DEBUG) cerr << "ATR -> LVALUE_ATR EQUAL RVALUE" << endl; $$ = $1 * $3 * "=" * "^"; }
    ...
    | LVALUEPROP_ATR PLUS_EQUAL RVALUE { if(DEBUG) cerr << "ATR -> LVALUEPROP_ATR PLUS_EQUAL RVALUE" << endl; $$ = $1 * $1 * "[@]" * $3 * "+" * "[=]" * "^"; }
    | LVALUEPROP_ATR EQUAL RVALUE { if(DEBUG) cerr << "ATR -> LVALUEPROP_ATR EQUAL RVALUE" << endl; $$ = $1 * $3 * "[=]" * "^"; }
    ;

LVALUE_ATR : ID { if(DEBUG) cerr << "LVALUE_ATR -> ID" << endl; setVariable($1); $$ = $1; }
           ;

LVALUEPROP_ATR : LVALUE FIELDS { if(DEBUG) cerr << "LVALUEPROP_ATR -> ID FIELDS" << endl; useVariable($1); $$ = $1 * "@" * $2; }
               ;

LVALUE : ID { if(DEBUG) cerr << "LVALUE -> ID" << endl; useVariable($1); $$ = $1; }
       | ID PLUS_PLUS { if(DEBUG) cerr << "LVALUE -> ID PLUS_PLUS" << endl; useVariable($1); $$ = $1 * $1 * "@" * "1" * "+" * "=" * "1" * "-"; }
       | OPEN_PAR LVALUE CLOSE_PAR { if(DEBUG) cerr << "LVALUE -> OPEN_PAR LVALUE CLOSE_PAR" << endl; $$ = $2; }
       ;

LVALUEPROP : LVALUE { if(DEBUG) cerr << "LVALUEPROP -> LVALUE" << endl; $$ = $1; }
           | LVALUEPROP FIELDS { if(DEBUG) cerr << "LVALUEPROP -> LVALUEPROP FIELDS" << endl; useVariable($1); $$ = $1 * "@" * $2; }
           | OPEN_PAR LVALUEPROP CLOSE_PAR { if(DEBUG) cerr << "LVALUE -> OPEN_PAR LVALUE CLOSE_PAR" << endl; $$ = $2; }
           ;
...
EXPR : NUM { if(DEBUG) cerr << "EXPR -> NUM" << endl; $$ = $1; }
     ...
     | LVALUE { if(DEBUG) cerr << "EXPR -> LVALUE" << endl; $$ = $1 * "@"; }
     | LVALUEPROP { if(DEBUG) cerr << "EXPR -> LVALUEPROP" << endl; $$ = $1 * "[@]"; }
